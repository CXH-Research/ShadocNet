...
class PDF(FPDF):
    ...
    def addInfo(self, codeText, s1, s2, w, h, itemCount):
        ...
        if itemCount == 0:
            # 4.0, 4.0
            # print(s1, s2)
            self.image("qrCodes/" + codeText + ".png", x=s1, y=s2, link='', type='', w=20, h=20)
        # elif itemCount == 4:
        #     # 168.0, 4.0
        #     print(s1, s2)
        ...
        #     self.image("qrCodes/"+codeText+".png", x=s1,y=s2+23,link='', type='', w=20, h=20)
            ...
            # 168.0, 256.0
            self.image("qrCodes/" + codeText + ".png", x=s1 + 24, y=s2 + 23, link='', type='', w=20, h=20)
...
with open("qr_codes.pdf", "rb") as inFile, open("test.pdf", "rb") as overlay:
    original = pypdf.PdfFileReader(inFile)
    foreground = pypdf.PdfFileReader(overlay)

    qr_code = original.getPage(0)

    # add all pages to a writer
    writer = pypdf.PdfFileWriter()
    for i in range(foreground.getNumPages()):
        new_page = pypdf.pdf.PageObject.createBlankPage(None, qr_code.mediaBox.getWidth(), qr_code.mediaBox.getHeight())
        new_page.mergePage(qr_code)
        text = foreground.getPage(i)
        new_page.mergePage(text)
